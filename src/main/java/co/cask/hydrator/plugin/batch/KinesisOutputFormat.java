...
public class KinesisOutputFormat extends OutputFormat {
  ...
  public void checkOutputSpecs(JobContext context) throws IOException, InterruptedException {
    ...
    BasicAWSCredentials awsCred = new BasicAWSCredentials(conf.get(Properties.KinesisSink.ACCESS_ID),
                                                          conf.get(Properties.KinesisSink.ACCESS_KEY));

    if (kinesisClient == null) {
      LOG.info("Creating amazon kinesis client for stream {}", conf.get(Properties.KinesisSink.NAME));
      try {
        kinesisClient = new AmazonKinesisClient(awsCred);
      } catch (Throwable e) {
        LOG.error("Got error while creating the amazon client", e);
        throw e;
      }
      KinesisUtil.createAndWaitForStream(kinesisClient, conf.get(Properties.KinesisSink.NAME),
                                         Integer.valueOf(conf.get(Properties.KinesisSink.SHARD_COUNT)));
      LOG.info("Done creating amazon kinesis client for stream {}", Properties.KinesisSink.NAME);
    }
  }
  ...
  private class KinesisRecordWriter extends RecordWriter<NullWritable, Text> {
    ...
    KinesisRecordWriter(AmazonKinesisClient kinesisClient, TaskAttemptContext context) {
      ...
      this.distribute = context.getConfiguration().getBoolean(Properties.KinesisSink.DISTRIBUTE, true);
      this.streamName = context.getConfiguration().get(Properties.KinesisSink.NAME);
    }
